...
subtitle: ''
author: Tsuizen
description: 在学习Next.js13的新特性时发现部分浏览器已经支持了原生的图片懒加载，但却鲜有人提及。
tags:
  - 前端性能优化
  - HTML
featureImage: ''
...
事件运行在主线程并且触发频繁，需要再写一个节流函数，这种方式并不优雅。[使用实例](https://zhuanlan.zhihu.com/p/55311726)

第二种方式使用 `IntersectionObserver` API 监听页面元素的可见性，是目前最为推荐的一种方式。`IntersectionObserver` 更方便使用并且具有更好的可读性，
我们只需要考虑编写元素可见时需要执行何种操作的代码即可。[使用实例](https://web.dev/lazy-loading-images/)
